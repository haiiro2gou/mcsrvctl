...
export default async (client, input) => {
    try {
        ...
        const hub = (input[2].check ? `:green_circle: [Hub] - (${input[1].data.version.name})\n` : `:red_circle: [Hub] -\n`);
        const temp = (input[3].check ? `:green_circle: [Temp] ${input[2].data.motd.clean} (${input[2].data.version.name})\n` : `:red_circle: [Temp] -\n`);
        const event = (input[4].check ? `:green_circle: [Event] ${input[3].data.motd.clean} (${input[3].data.version.name})\n` : `:red_circle: [Event] -\n`);
